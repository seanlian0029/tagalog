// Excel is the editing source. Visitors read only the generated JSON files.
...
async function fetchSheetData(sheet, range = 'A:H') {
  ...
  if (!jsonPath) throw new Error(`No JSON file is configured for ${sheet}.`);
  return fetchJsonRows(jsonPath);
}
...
async function fetchDatabaseForId(id) {
  ...
  return fetchJsonRows(`data/database/${relativePath}`);
}
